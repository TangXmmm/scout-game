...
function renderTable(state) {
  ...
  const cx = 50;   // %
  const cy = 50;   // %
  // 椭圆半径：桌面内缩 20px(上下)，席位头像 42px，需要溢出桌面边缘
  // table-section: width=94vw(max 600px), height=clamp(210,36vw,280)px
  // 桌面 felt: top/bottom=20px，即桌面高约 170-240px
  // rx 稍大使席位分布在桌面左右端（扑克桌风格）
  const rx = 46;   // % 水平半径（较大，席位分布到桌面两端）
  const ry = 42;   // % 垂直半径（较大，席位溢出桌面上下边缘）
