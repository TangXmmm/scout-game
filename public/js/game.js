...
function renderPlayersTop(state) {
  ...
  bar.innerHTML = state.players.map(p => {
    ...
    const live = (p.scoutTokens || 0) - (p.handCount || 0);
...
function renderPlayersSidebar(state) {
  ...
  const players = state.players.map(p => {
    ...
    return `<div class="sidebar-player ${active ? 'active' : ''}">
      ...
      <div class="sp-score">
        总分 ${p.totalScore} · 🎫${p.scoutTokens || 0} · 🃏${p.handCount || 0}
      </div>
      <div class="sp-live">${live >= 0 ? '+' : ''}${live} 预计</div>
...
function confirmScout() {
  ...
  if (scoutAndShowMode) {
    // 暂存挖角参数，关面板让用户选牌
    window._pendingScout = { pos: selPos, insertIdx: selInsertIdx, flip: willFlip };
    closeScoutModal();
    showToast('✅ 挖角已选好！请在手牌区选择要演出的连续牌，再点演出按钮', 'info');
    const btn = document.getElementById('btn-show');
    btn.textContent = '⚡ 确认挖+演';
...
function doPendingScoutAndShow() {
  ...
  const p = window._pendingScout;
  socket.emit('scout_and_show', {
    scoutPosition: p.pos,
    insertIndex: p.insertIdx,
    showIndices: selectedIndices,
    flipCard: p.flip || false,
  });
  selectedIndices = [];
  resetShowBtn();
}
