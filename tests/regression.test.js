...
test('E-03 | 演出必须能压过原来的在场组（挖角前快照比较）', () => {
  // p1出2张顺子[5,6]；p2挖左端，然后只出1张，1张<原来2张 → 失败
  const game = makeGameWithHands([
  ...
  const prep = game.prepareScoutAndShow('p2', 'left', 0);
  assert(prep.success);
  // p2手牌变为：[{5,8}, {1,2}, {2,5}, {7,7}]
  // 出1张 vs 原来2张 → 1<2 → 失败
  const r = game.finishScoutAndShow('p2', [0]);
  assert(!r.success, '1张不能压过原来的2张在场组');
});
...
test('E-04 | Token在prepare阶段给出，finish失败时token已发出（设计决策）', () => {
  ...
  // finish失败（1张不能压2张）
  const r = game.finishScoutAndShow('p2', [0]);
  assert(!r.success, '应该失败');
  // 新流程：prepare成功后token即已给出（挖角行为已发生）
  // 不回滚token（这是当前两步流程的设计决策）
...
test('L-07 | Scout&Show后演出失败时，挖角行为已完成（pendingScoutAndShow清除需cancel）', () => {
  const game = makeGameWithHands([
    [card(5, 8), card(6, 9), card(9, 9)],
    [card(1, 2), card(2, 5)],
  ]);
  ...
  const r = game.finishScoutAndShow('p2', [0]); // 1张压不过2张
  assert(!r.success, '演出应失败');
  // 按现有设计：finish失败后pendingScoutAndShow仍有效，可以重新选牌演出
